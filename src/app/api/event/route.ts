...
const DEFAULT_EVENT_NAME = "KiloClaw Launch Event";
...
async function createEvent(name: string) {
  ...
  return {
    error: lastError ?? new Error("Failed to generate unique event code"),
  };
}
...
export async function GET(req: NextRequest) {
  ...
  if (presentview === "launch") {
    ...
    if (createError || !created) {
      return NextResponse.json(
        { error: "Failed to start new event" },
        { status: 500 },
      );
    }
  ...
  if (eventError || !event) {
    const { event: created, error: createError } =
      await createEvent(DEFAULT_EVENT_NAME);
    if (createError || !created) {
